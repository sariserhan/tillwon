...
The campaign constant now lives in `app/lib/currentCampaign.ts` (shared with the Official Rules
page so the two cannot drift). Delete that file and make `app/page.tsx` a client component reading
the query. **`app/rules/page.tsx` imports the same constants** — repoint it at
`api.campaigns.getActiveCampaign` in this step too, or the rules page will publish stale odds and
eligibility while the campaign page shows live values. Replace the top of `app/page.tsx`:
