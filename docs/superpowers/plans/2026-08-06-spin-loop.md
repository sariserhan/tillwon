...
Then replace every `CAMPAIGN.name` with `"TillWon"`, `CAMPAIGN.prizeTitle` with `prize.title`, `CAMPAIGN.sponsorName` with `sponsor.name`, `CAMPAIGN.status` with `campaign.status`, and `CAMPAIGN.noPurchaseStatement` with `rules.noPurchaseStatement`. Pass `columns={campaign.reelColumns}` to `SpinDeck`.
